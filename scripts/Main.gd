...
func _build_roads() -> void:
	...
	var curbs := StaticBody3D.new()
	curbs.name = "Curbs"
	add_child(curbs)
	_build_sidewalks(roads, curbs)
...
func _build_sidewalks(roads: Node3D, curbs: StaticBody3D) -> void:
	...
	for gx in GRID:
		for side in [-1.0, 1.0]:
			...
			for seg in _segments():
				...
				bs.size = Vector3(0.18, 0.10, slen)
				bc.position = Vector3(cx, 0.05, smid)
				bc.shape = bs
				curbs.add_child(bc)
	...
	for gz in GRID:
		for side in [-1.0, 1.0]:
			...
			for seg in _segments():
				...
				var bc := CollisionShape3D.new()
				var bs := BoxShape3D.new()
				bs.size = Vector3(hlen, 0.10, 0.18)
				bc.position = Vector3(hmid, 0.05, cz)
				bc.shape = bs
				curbs.add_child(bc)
